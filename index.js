...
async function fetchMeaning(text) {
    ...
    const inputData = text ?? document.getElementsByClassName("input-box")[0].value;
...
function getSavedWords() {
    ...
    for (var i = 0; i < localStorage.length; i++) {
        ...
        iconElement.innerHTML = `<div id="cancel-icon"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
        <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
        </svg></div>`
        chipElement.id = localStorage.key(i);
        chipElement.innerHTML = `<div class="saved_word_chip">${localStorage.key(i)}</div>`
        chipElement.append(iconElement)
        iconElement.addEventListener('click', ()=>{
            document.getElementsByClassName("input-box")[0].value = chipElement.id;
            localStorage.removeItem(chipElement.id);
            getSavedWords();
        })
        chipElement.addEventListener('click', ()=>{
            document.getElementsByClassName("input-box")[0].value = chipElement.id;
            fetchMeaning(chipElement.id);
        })
        saved_words_box.appendChild(chipElement);
    }
}



const wordMeaningChip = document.getElementById('word')
